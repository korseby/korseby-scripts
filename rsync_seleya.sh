...
function process() {
	...
	if [[ "$USER" == "novisad" ]] || [[ "$USER" == "root" ]] ; then
		echo "${EXCLUDE}" > /tmp/${NAME}_exclude.list
		${RSYNC} --exclude-from=/tmp/${NAME}_exclude.list --archive --xattrs --acls --delete --verbose --out-format="%o: %f (%b/%l)" /Users/novisad/ --rsh="ssh -p 22" "novisad@seleya:/Users/novisad/"
		ssh -p 22 novisad@seleya /Users/novisad/Music/fix_traktor.sh	
	fi
	
	if [[ "$USER" == "root" ]] ; then
		${RSYNC} --archive --xattrs --acls --delete --verbose --out-format="%o: %f (%b/%l)" /Applications/Zusatzprogramme/ --rsh="ssh -p 22" "admin@seleya:/Applications/Zusatzprogramme/"
	fi
}
